...
Der konfigurierte Wirkstoffanteil repraesentiert den aktiven Chloranteil des realen Produkts. Der Chlorprodukt-Typ (`organic` oder `inorganic`) steuert dabei nur die fachliche Einordnung fuer Stabilisator/CYA sowie den Default-Fallback bei fehlendem Wert. Aktuell faellt die Integration auf `0.56` fuer organische Produkte und `0.65` fuer anorganische Produkte zurueck.
