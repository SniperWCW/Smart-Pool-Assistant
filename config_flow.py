...
class SmartPoolAssistantConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def _get_schema(self, defaults=None):
        if defaults is None:
            defaults = {}
        return vol.Schema({
            vol.Optional(CONF_BLE_ADDRESS, default=defaults.get(CONF_BLE_ADDRESS)): selector.BluetoothSelector({}),
            vol.Optional(CONF_CHLOR_SENSOR, default=defaults.get(CONF_CHLOR_SENSOR)): selector.EntitySelector({"domain": "sensor"}),
            vol.Optional(CONF_PH_SENSOR, default=defaults.get(CONF_PH_SENSOR)): selector.EntitySelector({"domain": "sensor"}),
            vol.Optional(CONF_TEMP_SENSOR, default=defaults.get(CONF_TEMP_SENSOR)): selector.EntitySelector({"domain": "sensor"}),
